...
class PerformanceEvaluator:
    ...
    def _remove_profile_buffers(model: nn.Module):
        """移除 thop 注入的统计缓冲，避免污染 state_dict。"""
    ...
    def _build_injection_aware_benchmark_model(
        ...
        physical_layer: Optional[nn.Module],
        device: str,
        injection_grid_size: int = 16,
    ) -> nn.Module:
        """
        为需要 coeffs_map 注入的复原网络构建基准测试包装器。
        包装器在 forward 时自动生成 coeffs_map，并保持复原网络调用方式不变。
        """
        ...
        class _InjectionAwareWrapper(nn.Module):
            ...
            def forward(self, x):
                ...
                coeffs_map = None
                if needs_coeffs:
                    coeffs_map = self.phys_model.generate_coeffs_map(
                        h, w, self.run_device,
        ...
        wrapper = _InjectionAwareWrapper(
            restoration_net,
            physical_layer,
            injection_grid_size,
            device,
    ...
    def evaluate(self, restoration_net: nn.Module, physical_layer: Optional[nn.Module], val_loader, device: str,
        ...
        if requires_physics_injection:
            if not use_physical_layer:
                ...
                benchmark_model = self._build_injection_aware_benchmark_model(
                    restoration_net,
                    physical_layer,
                    device,
                    injection_grid_size=injection_grid_size,
                )
